...
from aind_behavior_core_analysis.contract._core import DataStream, DataStreamCollection
...
class TestDataStream:
    ...
    def test_creation(self, text_file):
        ...
        stream = SimpleDataStream(
            name="test", 
            description="Test stream", 
            reader_params=SimpleParams(path=text_file)
        )
...
class TestDataStreamCollection:
    ...
    def test_creation(self, text_file):
        ...
        stream1 = SimpleDataStream(
            name="stream1", 
            reader_params=SimpleParams(path=text_file)
        )
        stream2 = SimpleDataStream(
            name="stream2", 
            reader_params=SimpleParams(path=text_file)
        )
    ...
    def test_walk_data_streams(self, text_file):
        ...
        inner_collection = DataStreamCollection(
            name="inner",
            data_streams=[stream2]
        )
        
        outer_collection = DataStreamCollection(
            name="outer",
            data_streams=[stream1, inner_collection]
        )
