...
class TestJson:
    ...
    def test_read_json(self, json_file):
        ...
        json_stream = Json(
            name="test",
            reader_params=JsonParams(path=json_file)
        )
...
class TestMultiLineJson:
    ...
    def test_read_jsonl(self, jsonl_file):
        ...
        jsonl_stream = MultiLineJson(
            name="test",
            reader_params=JsonParams(path=jsonl_file)
        )
...
class TestPydanticModel:
    ...
    def test_read_pydantic(self, temp_dir):
        ...
        model_stream = PydanticModel(
            name="test",
            reader_params=PydanticModelParams(
                path=file_path,
                model=MockModel
            )
        )
    ...
    def test_with_column_renaming(self, temp_dir):
        ...
        models_stream = ManyPydanticModel(
            ...
            reader_params=ManyPydanticModelParams(
                path=file_path,
                model=MockModel,
                column_names={"name": "subject_name", "value": "score"}
            )
        )
